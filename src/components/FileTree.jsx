      ...
      <div className='text-gray-600 group inline-block'>
        {
          state.dirfiles?.sort((a, b) => a.name.split('-')[0] - b.name.split('-')[0]).map(dir => {
            return (
              <details className='pl-3' key={dir.name} open={decodeURI(location.pathname).includes(dir.name) || open[`${state.username}-${dir.name}`]} onToggle={e => handleToglle(e, dir.name)}>
